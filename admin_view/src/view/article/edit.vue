...
watch(() => route.params.id, (id) => {
  if (id !== undefined){
    articleData.value.id = Number(id)
    getInfoArticle()
  }
})
...
const getInfoArticle = async()=>{
  const articleDateRep =  await infoArticle({
    ID:articleData.value.id,
  })
  if(articleDateRep.code === 0){
    articleData.value = articleDateRep.data.infoArticle
  }
}
  ...
  methods: {
    submitForm() {
      this.$refs['article'].validate(async valid => {
        if (!valid) return
        // TODO 提交表单
        const res = await editArticle(this.articleData)
        if (res.code === 0 ){
    ...
    resetForm() {
      this.$refs['article'].resetFields({id: Number(route.params.id)})
    },
